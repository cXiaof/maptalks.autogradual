...
function isSymbol(value) {
  return (typeof value === 'undefined' ? 'undefined' : _typeof$8(value)) == 'symbol' || isObjectLike_1(value) && _baseGetTag(value) == symbolTag$1;
}
...
var AutoGradual = function (_maptalks$Class) {
    _inherits(AutoGradual, _maptalks$Class);

    function AutoGradual(options) {
        _classCallCheck(this, AutoGradual);
    ...
    AutoGradual.prototype._sortGeosByExtent = function _sortGeosByExtent(geos) {
        var obj = [];
        var directionX = this.options['direction'].toLowerCase() === 'x';
        var directionY = this.options['direction'].toLowerCase() === 'y';
        if (!directionX && !directionY) return geos;
        geos.forEach(function (geo) {
            var _geo$getExtent = geo.getExtent(),
                xmin = _geo$getExtent.xmin,
                ymax = _geo$getExtent.ymax;

            var sortKey = directionY ? ymax : xmin;
            obj.push({ sortKey: sortKey, geo: geo });
        });
        var sortResult = sortBy_1(obj, 'sortKey');
        if (directionY) sortResult = reverse_1(sortResult);
        var sortGeos = [];
        sortResult.forEach(function (item) {
            return sortGeos.push(item.geo);
        });
        return sortGeos;
    };

    AutoGradual.prototype._doRadualChange = function _doRadualChange() {
        var _this2 = this;

        var target = this.options['colors'];
        ...
        if (this._length >= length) {
            ...
            var tasks = this._getTasksIndexArrBygap(gaps);
            tasks.forEach(function (task, index) {
                var colorArr = _this2._getItemsColor(target[index], target[index + 1], task.length);
                task.forEach(function (key, i) {
                    return _this2._updateGeosSymbol(_this2.geometries[key], colorArr[i]);
                });
            });
        } else this.geometries.forEach(function (geo, index) {
            return _this2._updateGeosSymbol(geo, _this2.options['colors'][index]);
        });
    };

    AutoGradual.prototype._getTasksIndexArrBygap = function _getTasksIndexArrBygap(gaps) {
    ...
    AutoGradual.prototype._updateGeosSymbol = function _updateGeosSymbol(geo, color) {
    ...
    AutoGradual.prototype._getItemsColor = function _getItemsColor(colorStart, colorEnd, level) {
        colorStart = this._getColorValueArr(colorStart);
        colorEnd = this._getColorValueArr(colorEnd);
        var arr = [];
        var _colorStart = colorStart,
            sR = _colorStart[0],
            sG = _colorStart[1],
            sB = _colorStart[2];
        var _colorEnd = colorEnd,
            eR = _colorEnd[0],
            eG = _colorEnd[1],
            eB = _colorEnd[2];


        var perR = (eR - sR) / level;
        var perG = (eG - sG) / level;
        var perB = (eB - sB) / level;

        for (var i = 0; i <= level; i++) {
            var r = parseInt(perR * i + sR, 0);
            var g = parseInt(perG * i + sG, 0);
            var b = parseInt(perB * i + sB, 0);
            arr.push('rgb(' + r + ',' + g + ',' + b + ')');
        }
        return arr;
    };

    AutoGradual.prototype._getColorValueArr = function _getColorValueArr(color) {
        var arr = [];
        try {
            ...
            color.split(',').forEach(function (item) {
                return arr.push(parseInt(item, 0));
            });
        }
        return arr;
    };
...
exports.AutoGradual = AutoGradual;

Object.defineProperty(exports, '__esModule', { value: true });

typeof console !== 'undefined' && console.log('maptalks.autogradual v0.1.0-alpha.1');
